...
const log = debug("siren-nav:steps:follow");
...
export function findPossible(
  ...
  (siren.entities || []).forEach(entity => {
    ...
    if (entity.hasOwnProperty("href")) {
      ...
      log("  Found possible match in subentity link, href = %s", href);
  ...
  links.forEach((link: Link) => {
    ...
    log("  Found possible match among links: %j", link);
...
export function followLocation(baseConfig: Config) {
  return async (state: NavState) => {
    log("Following Location header");
    const resp = await getRequest(state);
    log("Response for %s was: %j", state.cur, resp);
    log("  Headers: %j", resp.headers);
    let location = resp.headers["Location"] || resp.headers["location"];
    if (!location) {
      log(
        "  ERROR: No 'Location' header found, payload for %s was %j",
        state.cur,
        resp.data
      );
      throw new Error(
        "No 'Location' header found in '" +
          Object.keys(resp.headers).join(", ") +
          "'"
      );
    } else {
      log("  Location header: %s", location);
    }
  ...
  return (state: NavState): Promise<NavState> => {
    return getSiren(state).then(siren => {
      ...
      if (possible.length == 0) {
        console.error(
          "Cannot follow relation '" + rel + "', no links with that relation"
        );
        throw new Error(
          "Cannot follow relation '" + rel + "', no links with that relation"
        );
      ...
      log("  Found match, resulting state: %j", possible[0]);
      return possible[0];
    });
  };
}
